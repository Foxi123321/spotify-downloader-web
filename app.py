...
def get_yt_dlp_opts(is_search=False):
    ...
    common_opts = {
        ...
        'retry_sleep_functions': {
            '403': lambda _: random.uniform(2, 4),
            '429': lambda _: random.uniform(2, 4),
            'generic': lambda _: random.uniform(1, 3)
        },
        ...
        try:
            with yt_dlp.YoutubeDL(get_yt_dlp_opts(is_search=True)) as ydl:
                ...
                if result and 'entries' in result and result['entries']:
                    ...
                    video = random.choice(result['entries'][:3])
                    if video:
...
def process_download(download_id, track_info, youtube_url):
    try:
        ...
        if 'temp_dir' in downloads[download_id] and os.path.exists(downloads[download_id]['temp_dir']):
            try:
